...
class Builder(BasicWorker):
    ...
    def __init__(self,
        ...
        self.send_binary_method = send_binary_method
    ...
    def get_clone_dir(self, repo):
        ...
        hashedurl = hashedurl + ''.join(random.choice(string.ascii_lowercase) for _ in range(10))
    ...
    def clone(self, repo, clone_dir):
        ...
        try:
            ...
            tmp_file_path = os.path.join(self.tmp_dir, hashlib.md5(repo["url"].encode()).hexdigest()+''.join(random.choice(string.ascii_lowercase) for _ in range(5))+".zip")
        ...
        if clone_status == BuildStatus.SUCCESS:
            ...
            for library, mode, version, opti in build_task_configs:
                ...
                if self.platform == "windows" and build_status == BuildStatus.SUCCESS:
                    ...
                    if self.send_binary_method.lower() == "ftp":
                        build_method.post_processing_ftp(
                            self.server_addr, file_location, repo, zip_file)
                        self.send_msg("post_analysis.ddisasm", repo,
                                      file_name=f"/binaries/ftp/{zip_file}")
                    elif self.send_binary_method.lower() == "s3":
                        logging.info("Posting binary `%s` to S3.", repo_fname)
                        s3_file = build_method.post_processing_s3(repo_fname, file_location)
                        if s3_file:
                            self.send_msg("post_analysis.ddisasm", repo, file_name=s3_file)
