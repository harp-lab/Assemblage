...
aws_profile = AWSProfile("assemblage-vcpkg", "default")
...
class VcpkgBuild(BuildStartegy):
	...
	def clone_data(self, repo) -> tuple[bytes, int, str]:
		...
		dest_path = f"Builds/{repo['url']}_{optimization_level}"
	...
	def run_build(self, repo, target_dir, build_mode, library, optimization,
		...
		cmd = f"vcpkg install {repo['url']} --overlay-triplets={target_dir}/triplets --x-install-root={target_dir} --allow-unsupported --triplet x64-windows"
		logging.info(cmd)
		os.system(cmd)
		...
		with open(triplet_path, "w") as f:
			for x in triplet_flags:
				f.write(f"set({x} {triplet_flags[x]})\n")
		cmd = f"vcpkg install {repo['url']} --overlay-triplets={target_dir}/triplets --x-install-root={target_dir} --allow-unsupported --triplet x86-windows"
		logging.info(cmd)
		os.system(cmd)
...
test_cluster_vcpkg = AssmeblageCluster(name="test", coordinator_addr="52.53.200.15"). \
				build_system_analyzer(vcpkg_build_sys_hook). \
				aws(aws_profile). \
				message_broker("52.53.200.15"). \
				builder(
				...
				build_option(
					...
					compiler_name="vcpkg",
					compiler_flag=optimization_level,
					build_system="vcpkg")
