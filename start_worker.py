...
from assemblage.worker.disasm import DDisasmWorker
...
if __name__ == '__main__':
    ...
    if args.config:
        ...
        s3_client.download_file(args.s3, f"{instance_tag}.config", local_file_name)
            ...
            with grpc.insecure_channel(config['grpc_addr']) as channel:
                if os.name == 'nt':
                    ...
                    with open('/conf', 'r') as f:
                        opt_parts = f.read().strip().split(':')
                        opt_id = int(opt_parts[0].strip())
                        compiler = opt_parts[1].strip()
                    worker = Builder(
                        config['rabbitmq_host'],
                        config['rabbitmq_port'],
